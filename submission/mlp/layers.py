...
class DropoutLayer(StochasticLayer):
    ...
    def fprop(self, inputs, stochastic=True):
        ...
        if (self.share_across_batch):
            tmpMask = 1 * ( self.rng.uniform(size = inputs.shape[1:]) < self.incl_prob)
            self.mask = np.array([tmpMask for j in range(0,inputs.shape[0])])
        else:
            self.mask = 1 * (self.rng.uniform(size = inputs.shape) < self.incl_prob)

        if(stochastic):
            return np.multiply(self.mask,inputs)
        else:
