...
\youranswer{
the train and validation accuracy initially increase as a function of epoch number. The same behaviour can be seen in figure 1b with the train and validation accuracy. At approximately 10 epochs one can see a sharp turn in the validation error. From this point on the 2 errors diverge. This is quite typical of overparamaterised networks and is indicative of over-fitting. From this point on the train validation loss gap increases with epoch number. Even when a network is generalising well, one does still expect to see a small gap between the train and validation error. The fact that the validation error is increasing does however indicate that the model is further delving into the over-fitting regime. }
}
